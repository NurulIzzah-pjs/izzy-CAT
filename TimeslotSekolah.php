...
function isMykadAlreadyBooked($mysqli, $mykad) {
    $stmt = $mysqli->prepare("SELECT COUNT(*) FROM psj_bookingsekolah WHERE user_id = ?");
...
$sql = "SELECT timeslot, available_slots FROM psj_timeslotsekolah";
    ...
    function bookTimeslot(timeslot, mykad, availableSlots) {
        ...
        if (confirmation) {
            if (availableSlots > 0) {
                ...
                xhr.open("GET", 'BookedslotSekolah.php?timeslot=' + encodeURIComponent(timeslot) + '&mykad=' + encodeURIComponent(mykad), true);
