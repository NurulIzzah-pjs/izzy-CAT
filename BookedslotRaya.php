...
if ($_SERVER["REQUEST_METHOD"] ?? '' == "GET" && isset($_GET['timeslot']) && isset($_GET['mykad'])) {
    ...
    if (!empty($mykad)) {
        ...
        $checkStmt = $mysqli->prepare("SELECT available_slots FROM psj_timeslotraya WHERE timeslot = ?");
        ...
        if ($availableSlots > 0) {
            $updateStmt = $mysqli->prepare("UPDATE psj_timeslotraya SET available_slots = available_slots - 1 WHERE timeslot = ?");
            $updateStmt->bind_param('s', $time_slot);
            $updateStmt->execute();
            $updateStmt->close();

            $insertStmt = $mysqli->prepare("INSERT INTO psj_bookingraya(User_ID, timeslot) VALUES (?, ?)");
            if ($insertStmt) {
